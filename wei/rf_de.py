...
def assign(i):
  global The
  The.rf.min_samples_split = i[0]
  The.rf.min_samples_leaf = i[1]
  The.rf.max_leaf_nodes = i[2]
  The.rf.n_estimators = i[3]
  The.option.threshold = i[4] # use the.option.threshold for threshold!
  The.rf.max_features = i[5]

def generate():
  candidates = [r(l,h) if isinstance(l, float) else ig(l,h)for l,h in zip(Settings.de.rfLimit_Min, Settings.de.rfLimit_Max)]
  # candidates = treat(candidates)
  return candidates

def trim(i, x):
  return max(Settings.de.rfLimit_Min[i], min(int(x),Settings.de.rfLimit_Max[i]))
...
def writeResults(bestscore, evaluation):
  # pdb.set_trace()
  name = ['bestscore','evaluation','threshold','max_features', 'max_leaf_nodes', 'min_samples_split', 'min_samples_leaf',  'n_estimators']
  val =[str(bestscore), str(evaluation), str(The.option.threshold),str(The.rf.max_features), str(The.rf.max_leaf_nodes), 
       str(The.rf.min_samples_split), str(The.rf.min_samples_leaf), str(The.rf.n_estimators)] 
